...
class Memorybank:
    def __init__(self,vectorize,sentences = None,Labels = None):
        ...
        self.Labels = Labels
        neg_ins = open('./corpora/ECSIN-NEG/negative_instance_ECSIN.txt','r',encoding='utf-8')
        self.entities = neg_ins.readlines()
        ...
        for cnt in tqdm(range(len(self.Labels))):
            ...
            if self.Labels[cnt] != 'noncause':
                ...
                for i in range(0,len(keys)):
                    ...
                    neg_num = random.sample(set(range(0,len(self.entities))),2)
                    random_key = neg_num[0]
                    random_value = neg_num[1]
                    sent2_neg = self.entities[random_key].strip('\n') + ' ' + connective + ' ' + self.entities[random_value].strip('\n')
