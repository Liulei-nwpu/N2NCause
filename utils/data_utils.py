...
class DatasetVectorizer:
    ...
    def __init__(self, model_dir, char_embeddings, raw_sentence_pairs=None, save_vocab=True):
        ...
        if self.raw_sentence_pairs is None:
            ...
            self.raw_sentence_pairs = [str(x) for x in list(self.raw_sentence_pairs)]
            if self.char_embeddings:
                log('Chosen char embeddings.')
                self.sentences_lengths = [len(list(str(x))) for x in list(self.raw_sentence_pairs)]
            else:
                log('Chosen word embeddings.')
                self.sentences_lengths = [len(str(x).split(' ')) for x in list(self.raw_sentence_pairs)]
            ...
            self.vocabulary.fit(self.raw_sentence_pairs)
            if self.save_vocab:
